  ...
  for (const action of plan) {
    try {
      if (action.type === "exit") {
        ...
        const text = `Auto ${label}: sold ${action.symbol} (${action.pnlPct.toFixed(1)}% from entry)`;
        summaries.push(text);
        ...
        traded = true;
        const text = `Auto buy: ${action.quantity} ${action.symbol} on watchlist dip`;
        summaries.push(text);
        await appendDeskEvent(userId, {
          kind: "auto-entry",
          text,
        ...
        attentionOnly = true;
        const text = `Auto held off on ${action.symbol}: ${action.reason}`;
        summaries.push(text);
        await appendDeskEvent(userId, {
          kind: "auto-skip",
          text,
      ...
      attentionOnly = true;
      const text = `Auto couldn't finish on ${alert.snapshot.symbol}: ${message}`;
      summaries.push(text);
      await appendDeskEvent(userId, {
        kind: "auto-skip",
        text,
