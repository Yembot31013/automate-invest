...
const OTHER_TOOL_INTENT =
  /\b(monitor|watch(?:list)?|track|pin(?:ned)?|unmonitor|remove\s+from|stop\s+watching|paper\s+buy|paper\s+sell|\bbuy\b|\bsell\b|close\s+(?:my|the)\s+(?:position|paper)|recommend|portfolio|p\s*&\s*l|pnl|what[\s-]?if|list\s+watchlist|what\s+are\s+you\s+watching|naira|ngn|dollar\s+rate|fx\s+rate|convert|auto[\s-]?trade|attention\s+mail)\b/i;
