...
- Arming price triggers on watchlist FX: if the pair is already on the live watchlist, call createTrigger directly — do NOT require getSnapshot first. One Yahoo blip can fail verify; that is transient, not "feed offline". If createTrigger fails once, retry createTrigger once before calling getSnapshot. Never declare the data feed dead from a single timeout.
