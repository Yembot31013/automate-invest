...
- Triggers: user-defined standing rules — same options as Add → Trigger (When: day drop % / day gain % / price ≤ / price ≥; Then: alert me / paper buy / paper sell). Threshold is always a positive number (3 = −3% day for day_drop_pct). Paper buy needs a USD size (notionalUsd, default $1k) and enough cash; paper sell needs an open lot. Each rule can set autoPauseAfterFire (default on for paper buy/sell): pauses after a successful fire. Max ${MAX_USER_TRIGGERS}. Use createTrigger / updateTrigger / listTriggers / setTriggerEnabled / removeTrigger — do NOT create a duplicate when they want to change an existing rule; listTriggers, pick the right id (if several on one symbol, ask or use summaries). Example: “buy when day drops 3%” = createTrigger …; “change my TSLA trigger to 2%” = updateTrigger. Vague “any negative / goes red / when it dips” is NOT value 0 — ask for a concrete % before arming.
